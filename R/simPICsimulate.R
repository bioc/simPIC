    ...
    #'  \item Set up default parameters if needed
    #'  \item Set up SingleCellExperiment object
    #'  \item Simulate library sizes
    #'  \item Simulate accessibility probability
    ...
    #'     \item{\code{rowData}}{
    #'         \describe{
    #'             \item{PeakMean}{The simulated peak means.}
    #'         }
    #'     }
    #'     \item{\code{colData}}{
    #'         \describe{
    ...
    simPICsimulatePeakMean <- function(object, sim, pm.distr, verbose) {
        ...
        if (pm.distr == "gamma") {
            message("using gamma distribution for simulating peak mean")
            peak.mean.shape <- simPICget(object, "peak.mean.shape")
            peak.mean.rate <- simPICget(object, "peak.mean.rate")
            peak.means <- rgamma(
                n = nPeaks,
                shape = peak.mean.shape,
                rate = peak.mean.rate
            )
        } else if (pm.distr == "weibull") {
            message("using weibull distribution for simulating peak mean")
            peak.mean.shape <- simPICget(object, "peak.mean.shape")
            ...
            message("using pareto distribution for simulating peak mean")
            peak.mean.shape <- simPICget(object, "peak.mean.shape")
            peak.mean.scale <- simPICget(object, "peak.mean.scale")
            peak.means <- rpareto(
                n = nPeaks,
            ...
            message("using lngamma distribution for simulating peak mean")
            ...
            peak.means <- rlngamma(
                ...
                pi = peak.mean.pi,
                shape = peak.mean.shape,
                rate = peak.mean.shape,
                meanlog = peak.mean.meanlog,
    ...
    if (nPeaks > length(sparsity)) {
        ...
        sparsity <- sample(seq(sparsity.min, sparsity.max), nPeaks, replace = TRUE)
        }
