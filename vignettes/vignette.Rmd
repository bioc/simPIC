...
simPIC. Here is an example a mock dataset generated with the `mockATAC` function
from `simPIC` package:
...
own matrix to PIC by following the
[PICsnATAC](https://github.com/Zhen-Miao/PICsnATAC/blob/main/vignettes/)
...
```{r getParams-setParams}
    ...
    params <- simPICgetparameters(sim.params,
...
```{r simPICestimate}
    ...
    count <- readRDS("../inst/extdata/test.rds")
